...
/**
 * A class responsible for handling enquiry-related data in Excel format.
 * Implements the IEnquiriesData interface.
 */

public class EnquiryExcelData implements IEnquiriesData {
	@Override
	public void save(EnquiryList enquiryList) {
        ...
        try {
            ...
            // Load the Excel file
            Workbook workbook = WorkbookFactory.create(new FileInputStream("src/excel/enquiry_list.xlsx"));
            ...
            for(int i=1; i<enquiryList.size()+1;i++) {
	        	...
	        	cell5.setCellValue(enquiryList.get(i-1).getStatus());
	        	...
	        	cell7.setCellValue(enquiryList.get(i-1).getReply());
            	
            }   
            // Now, you can save the changes back to the Excel file
        	try (FileOutputStream fos = new FileOutputStream("src/excel/enquiry_list.xlsx")) {
        	    workbook.write(fos);
        	}      
            // Close the workbook
            workbook.close();
