...
/**
 * The SuggestionExcel class implements the ISuggestionData interface for handling suggestions using Excel.
 * It provides methods for reading, writing, and manipulating suggestion data stored in an Excel file.
 */

public class SuggestionExcel implements ISuggestionData {
	...
	public void save(SuggestionList suggestionList) {
        ...
        try {
            // Load the Excel file
            Workbook workbook = WorkbookFactory.create(new FileInputStream("src/excel/suggestion_list.xlsx"));
        	...
        	try (FileOutputStream fos = new FileOutputStream("src/excel/suggestion_list.xlsx")) {
        	    workbook.write(fos);
        	}      
            // Close the workbook
            workbook.close();
